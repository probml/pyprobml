...
import os
from .linreg_1d_plot_demo import *
...
def run_expt(config, loss_opt=0):
    ...
    Xtrain, Ytrain, params_true, true_fun, fun_name = \
      make_data_linreg_1d(config['N'], config['fun_type'])
    ...
    if config['optimizer'] == 'SGD':
        B = config['batch_size']
        M = int(N / B) # num_minibatches_per_epoch (num iter per epoch)
    ...
    if plot_params:
        ...
        plot_error_surface_2d(loss_fun, params, params_true, config['fun_type'], ax)
