function [theta, para_post, x_post_all_iter] = inference_modified(M, pop, obs_truth, OEV, ...
    num_ens, Iter, num_times, rnds, legacy)
% theta is nparams * Iter+1 where nparams=6
% para_post is nparams * num_ens * num_times * Iter
% x_post is nvar * num_ens * num_times * Iter,
% where nvar = 1881 = 1875 + 6, where 1875 = 375*5
...
x_post_all_iter=zeros(num_var,num_ens,num_times,Iter);%KPM
for n=1:Iter
    sig(n)=alp^(n-1);
