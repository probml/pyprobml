...
if __name__ == "__main__":
    ...
    objective = make_potential(potential_key, predict, train_ds, batch_size, l2_regularizer)

    losses = jnp.array([])
    learning_rate = 1e-3
    optimizer = optax.adam(learning_rate)
    n_steps = 300
    anchor_params_tree, loss, _ = optimize_loop(objective, params_tree_init, optimizer, n_steps=n_steps, callback=None)
    print(f"Loss : {loss[-1]}")
