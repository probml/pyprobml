...
def generate_x_true(rng_key, K, x0_rvs: Callable, v_rvs: Callable, f: Callable):
    rng_keys = random.split(rng_key, num=2)
    x0 = x0_rvs(rng_keys[0], shape=())
    x_true = jnp.zeros(shape=(K + 1,))
...
def generate_y(rng_key, x_true, e_rvs: Callable, h: Callable):
    shape = x_true.shape
...
def kde(x_grid, x, kernel_variance):
    ...
    px = jax.vmap(functools.partial(jsp.stats.norm.pdf, loc=x, scale=kernel_variance))(
        x_grid
    )
...
def novel_density(
    ...
    p_filter = jnp.zeros(shape=(K + 1, num_grid_points))
    p_filter = p_filter.at[0, :].set(x0_pdf(x_grid))
    p_filter = p_filter.at[0, :].set(p_filter[0, :] / jnp.sum(p_filter[0, :]))
    p_pred = jnp.ones(shape=(K + 1, num_grid_points)) * jnp.inf
    rng_keys = random.split(rng_key, num=3)
    v = v_rvs(rng_keys[0], shape=(K + 1, num_samples))
    e = e_rvs(rng_keys[1], shape=(K + 1, num_samples))
...
def plot_densities(
    x_true, y, inv_h, x_grid, p_pred, p_filter, p_smooth, K, legend=True
):
    nplot = K + 1
    ncol = 10
    nrow = nplot // ncol + 1
    ...
    plt.suptitle("All K density plots to look for weird pattern")
...
def experiment_setup(
    ...
    x_true = generate_x_true(rng_subkey, K=K, x0_rvs=x0_rvs, v_rvs=v_rvs, f=f)
    ...
    if plot_xy:
        ...
        ax.plot(range(K + 1), x_true, label="True state", color="k")
        ax.plot(range(K + 1), y, label="Measurements", color="r")
...
def point_mass_experiment(
    ...
    p_filter, p_pred, p_smooth = point_mass_density(
        y, x_grid, x0_pdf, x_pdf=x_pdf, v_pdf=v_pdf, e_pdf=e_pdf, f=f, h=h
    )

    if plot_all_densities:
        # looking for weird density plot by plotting all K densities
        plot_densities(x_true, y, inv_h, x_grid, p_pred, p_filter, p_smooth, K)
