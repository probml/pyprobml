...
model_mlp = keras.wrappers.scikit_learn.KerasClassifier(
        make_model, nhidden=100, epochs=nepochs, batch_size=batch_size)

model_logreg_skl = LogisticRegression(C=1e5, solver='lbfgs', multi_class='multinomial')

models = []
models.append(('baseline', model_baseline))
models.append(('LR-keras', model_logreg_keras))
models.append(('LR-skl', model_logreg_skl))
models.append(('MLP', model_mlp))
...
for name, model in models:
  ...
  ylogits = logit(yprobs[:,1])
  eval_classifier(ytest, ypred, name)
  plot_curves(ytest, ylogits, name)
